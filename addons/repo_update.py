...
# Remove all file in folder after creating zip file
remove_files = True
########## End SETTINGS
...
def is_addon_dir( addon ):
    ...
    if addon == "_updates": return False
    else: return True
...
class Compressor:
   ...
   def master( self ):
       ...
       for addon in mydir:
               ...
               if is_addon_dir( self.addon_path ):
                       ...
                       print "Name:%s Done" % (self.addon_name, )
   ...
   def _create_compressed_addon_release( self ):
       ...
       mtss = md5.new( open( os.path.join( self.addon_path, zipname ) ).read() ).hexdigest()
	   
       open(os.path.join(self.addon_path, zipname + ".md5"),"w").write( mtss )
       ...
       for the_file in self.addon_folder_contents:
              ...
              # delete directories
              if not os.path.isfile( the_path ) and (remove_files == True):
              ...
              elif not ( ('addon.xml' in the_file) or ('hangelog' in the_file) or ('fanart' in the_file) or ('icon' in the_file) or (zipname in the_file)) and (remove_files == True):
                      os.remove( the_path )
